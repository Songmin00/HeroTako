...
public abstract class GoogleSheetParser<T> : ScriptableObject where T : new()
{
    ...
    [Header("타입명 행")]
    public int typeRow = 3;         // 타입 명시 행
    [Header("데이터 시작 행")]
    public int dataStartRow = 4;    // 데이터 시작 행
    ...
    private object ConvertValue(string value, Type targetType, string typeName)
    {
        ...
        {
            Debug.LogWarning($"[Parser] '{typeName}' 타입 변환 실패 (값: {value}): {e.Message}");
        }
    ...
    public T GetData(string id)
    {
        ...
        Debug.LogWarning($"[Parser] ID '{id}'를 찾을 수 없습니다.");
        return default;
    }
